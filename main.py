...
from otp_service import send_otp, verify_otp, is_email_verified
...
async def api_verify_otp(payload: VerifyOtpRequest):
    ...
    if not result["success"]:
        raise HTTPException(status_code=400, detail=result["message"])
    return {"success": True, "message": "Email verify ho gaya."}
...
async def api_submit_form(payload: FormSubmission):
    ...
    if payload.otp_verified_email != payload.proposer_email:
        raise HTTPException(
            status_code=400,
            detail="OTP verify kiya gaya email aur proposer ka email match nahi karta.",
        )

    # Re-confirm OTP was actually verified for this session (server-side truth,
    # not just trusting the value the browser sent back)
    if not is_email_verified(payload.otp_verified_email):
